...
- [ ] **T1: Dictionary domain in `shared`**
...
- [ ] **T2: Dictionary build script**
  - Files: `apps/extension/scripts/build-dictionary.mjs`, `apps/extension/scripts/fixtures/kaikki-sample.jsonl`, `apps/extension/scripts/build-dictionary.test.ts`, `apps/extension/public/dict/NOTICE`
  ...
  - Verify: `pnpm --filter @vocab-os/extension exec vitest run scripts/build-dictionary.test.ts` turns the fixture into the expected entries

- [ ] **T3: Offscreen dictionary engine**
...
- [ ] **T5: Options shows the dictionary**
...
- [ ] **T7: Docs and full check**
  ...
  - Verify: `pnpm typecheck && pnpm test && pnpm build`; manual in Chrome (needs a human): select "massive" offline → meanings by POS → Save → card translation holds all meanings; select "running" → "from run"; select a sentence → machine translation
