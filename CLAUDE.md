...
- **Translation runs in the offscreen document** (`src/offscreen/translator.ts`): the bundled dictionary (`public/dict/en-vi.json`, Vietnamese + English Wiktionary, CC BY-SA; rebuilt by `scripts/build-dictionary.mjs`) for words and phrases of ≤ 4 words, then Chrome's Translator API if `available`, else transformers.js `Xenova/opus-mt-en-vi` (q8, WASM). `env.backends.onnx.wasm.wasmPaths` is cleared on purpose, because the default CDN path is blocked by MV3 CSP and the Vite-bundled WASM is used instead. The manifest CSP needs `'wasm-unsafe-eval'`.
